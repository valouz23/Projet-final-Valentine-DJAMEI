...
class ProductRepository {
    ...
    {
        $sql = "SELECT name, price, stock, description FROM product WHERE type = 'fruit' ORDER BY name";
        return $this->connection->query($sql)->fetchAll();
    }

    public function findAllBook() : array
    {
        $sql = "SELECT title, author, price, stock, description FROM product WHERE type = 'livre' ORDER BY title";
        return $this->connection->query($sql)->fetchAll();
    }

    public function findBook(string $title) : array
    {
        $sql = "SELECT title, author, price, stock, description FROM product WHERE type = 'livre' AND title =" . $title;
        return $this->connection->query($sql)->fetchAll();
    }

    public function findFruit(string $name) : array
    {
        $sql = "SELECT name, price, stock, description FROM product WHERE type = 'fruit' AND name =" . $name;
        return $this->connection->query($sql)->fetchAll();
    }
